...
from func_obj import run as func_obj_run
from func_fbx import run as func_fbx_run
from func_blend import run as func_blend_run
...
async def process_row(row_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        try:
            ...
            pass
        # 3.3 Upload GLB + клик на knopka_vibor_type
        print(f"[{_ts()}] Загрузка GLB: {baza_id}.glb")
        await func_upload_run(page, steel_client, session.id, baza_id)
        # 3.4 Выбор OBJ + конвертация + скачивание
        await func_obj_run(page, baza_id)
        ...
        try:
            ...
            pass
        # 4.3 Upload GLB + клик на knopka_vibor_type
        print(f"[{_ts()}] Загрузка GLB: {baza_id}.glb")
        await func_upload_run(page, steel_client, session.id, baza_id)
        # 4.4 Выбор FBX + конвертация + скачивание
        await func_fbx_run(page, baza_id)
        await asyncio.sleep(config.WAIT_TIME)
        ...
        try:
            ...
            pass
        # 5.3 Upload GLB + клик на knopka_vibor_type
        print(f"[{_ts()}] Загрузка GLB: {baza_id}.glb")
        await func_upload_run(page, steel_client, session.id, baza_id)
        # 5.4 Выбор BLEND + конвертация + скачивание
        await func_blend_run(page, baza_id)
        await asyncio.sleep(config.WAIT_TIME)
