"""
Конвертация GLB в BLEND и скачивание результата.
"""
...
from config import MAX_TIME, WAIT_TIME, DOWNLOAD_DIR, knopka_blend, knopka_convert, knopka_download
...
async def run(page, baza_id: str):
    """
    Конвертация GLB → BLEND и скачивание ZIP.
    Схема:
    1. Ждать knopka_blend и клик
    2. Ждать WAIT_TIME
    ...
    """
    print(f"[{_ts()}] Начало конвертации в BLEND")

    # 1. Выбор формата BLEND
    print(f"[{_ts()}] 1. Выбор формата BLEND")
    await wait_for_element(page, knopka_blend, timeout=MAX_TIME)
    await page.click(knopka_blend)
    ...
    # 3. Ожидание пока кнопка Download получит РЕАЛЬНЫЙ URL скачивания
    print(f"[{_ts()}] 3. Ожидание готовности кнопки Download...")
    await page.wait_for_function("""
        () => {
            const el = document.querySelector('a#yj');
            if (!el) return false;
            // Ждём пока href станет реальным URL (содержит http)
            return el.href && el.href.startsWith('http') && !el.classList.contains('disabled');
        }
    ...
    target_path = os.path.join(DOWNLOAD_DIR, f"{baza_id}_blend.zip")
    ...
    try:
        ...
        print(f"[{_ts()}] ✓ Конвертация в BLEND завершена: {target_path}")

    except Exception as e:
        # Очистка частично скачанного файла
        if os.path.exists(target_path):
            os.remove(target_path)
        print(f"[{_ts()}] ✗ Ошибка конвертации BLEND: {type(e).__name__}: {e}")
        raise
