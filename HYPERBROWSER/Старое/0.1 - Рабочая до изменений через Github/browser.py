...
async def launch_browser() -> Tuple[object, Browser, BrowserContext, Page, Hyperbrowser, object]:
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] WebSocket: {session.ws_endpoint}")
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Страница создана в контексте Hyperbrowser")
...
async def release_browser(
    ...
    hyperbrowser_client: Optional[Hyperbrowser],
    session_id: Optional[str]
) -> None:
    """
    Корректно закрывает сессию Hyperbrowser и освобождает ресурсы.

    Args:
        playwright: объект playwright
        browser: объект browser
        hyperbrowser_client: Hyperbrowser клиент
        session_id: ID сессии для stop
    """
    try:
