...
import baza
import browser
import func_site_test_free
import func_prompt
import func_model_test_free
import func_interface_test_free
...
async def process_one_row(row_data):
    ...
    try:
        ...
        # 5. Сбор токенов
        tokens_count = await func_site_test_free.run(page)
        ...
        # 6. Выбор модели
        await func_model_test_free.run(page)

        # 7. Настройка интерфейса (4K, 16:9)
        await func_interface_test_free.run(page)

        # 8. Генерация
        await func_generate.run(page)

        # UPSCALE ВЫКЛ, А МОДЕЛЬ ТЕСТОВАЯ БЕСПЛАТНАЯ CHATGPT 1k
